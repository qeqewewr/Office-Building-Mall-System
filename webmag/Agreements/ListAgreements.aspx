<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ListAgreements.aspx.cs" Inherits="webmag_Agreements_ListAgreements" %>
 ...
 <head id="Head1" runat="server">  
  ...
  <body> 
    ...
    <div class="queryArea"  style="width:100%">
            <form action="ListAgreements.aspx?pageno=1" name="searchForm"
				...
				<fieldset>
					<legend style="color: #006699;"> 商城协议信息查询  </legend> 
					<span>协议名称查询：<input type="text" name="lookByName" id="lookByName" title="请输入协议名称"
						 value="<%=keyword %>" 
        ...
        <div>
			<table class="resultTable">
                ...
                <input type="button" value="" class="addBtn" title="添加"
						onClick="window.location.href='AddAgreement.aspx'">
                        <% }%>
				...
				<tr class="topTitle" align="center">
					...
					<td align="center">协议名称</td>
					<td align="center">更新时间</td>
					<td align="center" colspan="2">操 作</td>
				</tr> 

              <form name="pageControlForm" action="DeleteAgreements.aspx?page=<%= pageno %>" method="post"
                ...
                <%for (int i = 0; i < agreementsList.Count; i++)
                  { %>
						...
						<%--	<td><input type="checkbox" name="selectDel"
								value="<%=agreementsList[i].ID %>" onclick="isAllSelected(this,'sltAll','0')" />
							</td> --%>
							
							<td title="<%= agreementsList[i].Name %>" align="left"><%= agreementsList[i].Name%></td> 
							<td title="<%= agreementsList[i].UpdateTime %>"><%= agreementsList[i].UpdateTime%></td>
							
							<td><input type="button" value="" 
								onclick="window.location.href='UpdateAgreements.aspx?id=<%= agreementsList[i].ID%>&pageno=<%= pageno %>'"
								class="updateBtn" title="编辑"> 
                               <%-- <input type="button"
								value="" class="deleteBtn" title="删除"
								onclick="checkDeleteSingleItem('DeleteAgreements.aspx?id=<%= agreementsList[i].ID%>&page=<%= pageno %>')">--%>
                                <input type="button"
								value="" class="viewBtn" title="查看"
								onclick="window.location.href='ViewAgreements.aspx?id=<%= agreementsList[i].ID%>&pageno=<%= pageno %>'">
						    </td>
						</tr>
				<%} %>
