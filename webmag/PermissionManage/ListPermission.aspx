<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ListPermission.aspx.cs" Inherits="webmag_PermissionManage_ListPermission" %>
 ...
 <head id="Head1" runat="server">  
  ...
  <body> 
    ...
    <div class="queryArea"  style="width:100%">
            <form action="ListPermission.aspx?pageno=1" name="searchForm"
				...
				<fieldset>
					<legend style="color: #006699;"> 管理员信息查询  </legend> 
					<span>管理员用户名查询：<input type="text" name="lookByName" id="lookByName" title="请输入管理员用户名"
						 value="<%=keyword %>" 
...
<%--                <span align="right"><input
              ...
              <form name="pageControlForm" action="ListPermission.aspx?page=<%= pageno %>" method="post"
                  ...
                  { %>
						...
						<!--	<td>
							...
							<input type="checkbox" name="selectDel"
								value="<%=managerList[i].ID %>" onclick="isAllSelected(this,'sltAll','0')" />
							</td> 
							-->
							<td title="<%= managerList[i].userName %>" align="left"><%= managerList[i].userName%></td> 
							<%--<td title="<%= employeList[i].Department %>"><%= employeList[i].Department%></td>--%>
							
							<td><input type="button" value="" 
								onclick="window.location.href='UpdatePermission.aspx?id=<%= managerList[i].ID%>&pageno=<%= pageno %>'"
